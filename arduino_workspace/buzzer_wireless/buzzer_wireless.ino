...
const int MOTOR_PIN = 17;       // number of motor output pin

const char ID[] = "ODD";       // id tag for this board
...
bool buzzing = false;
int16_t x_accel, y_accel, z_accel, total_accel;
...
void loop()
{
  ...
  {
    message_in = (char) bleuart.read();
    if (message_in == '1' && buzzing == false){
      buzzing = true;
      start_time = millis();
    } else if (message_in == '0' && buzzing == true){
      buzzing = false;
      start_time = 0;
    }
  }

  // Change the motor state based on this
  if (buzzing) {
    if ((millis() - start_time) > 60000) {
      buzzing = false;
      start_time = 0;
    } else if ((millis() - start_time) % 800 < 400) {
  ...
  accelgyro.getAcceleration(&x_accel, &y_accel, &z_accel);

  // Fill a buffer with the data and send it
  message_out = String(millis()) + " " + String(x_accel);
  uint8_t buf[64];
  message_out.getBytes(buf, sizeof(buf));
  bleuart.write(buf, message_out.length());
