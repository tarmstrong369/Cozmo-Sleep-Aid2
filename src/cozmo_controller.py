...
from trackers import ButtonTracker
...
class CozmoController:
    ...
    def get_behavior(self, behavior_name):
        behavior_list = []
        self.button_tracker = ButtonTracker.ButtonTracker(testing=False)
        ...
        if behavior_name == "attentionw":
            ...
            self.cli.drive_wheels(lwheel_speed=80.0,rwheel_speed=80.0, duration=2.0)
            # Raise head.
            ...
            if i<j:
                while l<k:
                    if (self.button_tracker.get_update()):
                        print("wake up snooze activated")
                        self.logger.log("Wake up: Snooze 1")
                        self.cli.drive_wheels(lwheel_speed=0.0,rwheel_speed=0.0)
                        time.sleep(270) #snooze time in seconds (270s = 4.5min, for some reson the snooze always activates twice so a total snooze of 9min)
                        l=0
                        i=i+1
                    else:
                    #Spin
            ...
            if i<j:
                while l<k:
                    if (self.button_tracker.get_update()):
                        print("bedtime snooze actiavted")
                        self.logger.log("Bedtime: Snooze 1")
                        self.cli.drive_wheels(lwheel_speed=0.0,rwheel_speed=0.0)
                        time.sleep(270) # snooze time in seconds (270s = 4.5min, for some reson the snooze always activates twice so a total snooze of 9min)
                        l=0
                        i=i+1
